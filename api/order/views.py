...
# because this request will be made from another resource that's why we need CSRF exempt as well.
...
def add(request, id, token):
    ...
    if request.method == "POST":
        ...
        amount = request.POST['amount']  # will come from frontend
        products = request.POST['products']

        total_pro = len(products.split(',')[:-1])  # Total Products
